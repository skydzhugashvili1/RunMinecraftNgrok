...
MC_SERVER_URL=$(wget -qO- $VERSION_MANIFEST_URL | jq -r '.latest.release as $version | .versions[] | select(.id == $version) | .url')

# Fetch the download URL for the server JAR from the extracted URL
MC_SERVER_JAR_URL=$(wget -qO- $MC_SERVER_URL | jq -r '.downloads.server.url')
...
wget -O $MC_SERVER_JAR $MC_SERVER_JAR_URL
